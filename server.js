...
fastify.get("/", async function (request, reply) {
  try {
    ...
    if (!accountResponse.ok) {
      throw new Error(
        `API Error: ${accountResponse.status} - ${accountResponse.statusText}`
      );
    }

    const accountData = await accountResponse.json();
    const relevantBalances = accountData.balances.filter((balance) =>
    ...
    for (const coin of COINS) {
      ...
      if (coin !== "USDT") {
        const priceResponse = await fetch(
          `${BASE_URL}/api/v3/ticker/price?symbol=${coin}USDT`
        );
    ...
    console.log("Total USDT Value:", totalValue.toFixed(2));
